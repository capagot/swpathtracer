...
void SBVH::build() {
    if (primitives_->size() > 0) {
        ...
        for (unsigned long int i = 0; i < primitives_->size(); ++i) {
...
bool SBVH::intersect(const Ray& ray, IntersectionRecord& intersection_record, std::size_t& num_intersection_tests,
...
void SBVH::splitNode(std::unique_ptr<SBVHNode>& node, float root_aabb_area, const std::string& ident,
    ...
    for (int axis = 0; axis < 3; ++axis) {  // 0 = X axis, 1 = Y axis, 2 = Z axis
        ...
        for (unsigned long int i = 0; i < node->getNumReferences() - 1; ++i) {
            if (i == 0) {
        ...
        for (long int i = node->getNumReferences() - 2; i >= 0; --i) {
            if (i == static_cast<long int>(node->getNumReferences() - 2)) {
            ...
            float this_cost =
                SAH(i + 1, left_area[i], node->getNumReferences() - 1 - i, right_area[i], node->aabb_.getArea());
            // root_aabb_area);
    ...
    if (best_axis == -1) {  // this is a leaf node
        ...
        //             float node_axis_span = node->aabb_.getMax()[axis] - node->aabb_.getMin()[axis];
        ...
        //                     split_plane_list[i].offset_ = node->aabb_.getMin()[axis] + i * bin_width;

        //                 for (unsigned int i = 0; i < node->getNumReferences(); ++i) {
        //                     // compute the entry/exit point of the primitive over the bin list
        //                     int in_bin = static_cast<int>(
        //                         ((*node->primitive_ref_list_)[i].aabb_.getMin()[axis] - node->aabb_.getMin()[axis]) /
        //                         (node_axis_span + kHashingEpsilon) * kBinListSize);
        //                     int out_bin = static_cast<int>(
        //                         ((*node->primitive_ref_list_)[i].aabb_.getMax()[axis] - node->aabb_.getMin()[axis]) /
        //                         (node_axis_span + kHashingEpsilon) * kBinListSize);
        ...
        //                         switch (axis) {
        //                             case 0:
        //                                 clipping_aabb.setMin(glm::vec3(split_plane_list[k].offset_,
        //                                                                node->aabb_.getMin().y, node->aabb_.getMin().z));
        //                                 clipping_aabb.setMax(glm::vec3(split_plane_list[k + 1].offset_,
        //                                                                node->aabb_.getMax().y, node->aabb_.getMax().z));
        //                                 break;
        //                             case 1:
        //                                 clipping_aabb.setMin(glm::vec3(node->aabb_.getMin().x,
        //                                                                split_plane_list[k].offset_,
        //                                                                node->aabb_.getMin().z));
        //                                 clipping_aabb.setMax(glm::vec3(node->aabb_.getMax().x,
        //                                                                split_plane_list[k + 1].offset_,
        //                                                                node->aabb_.getMax().z));
        //                                 break;
        //                             case 2:
        //                                 clipping_aabb.setMin(glm::vec3(node->aabb_.getMin().x, node->aabb_.getMin().y,
        //                                                                split_plane_list[k].offset_));
        //                                 clipping_aabb.setMax(glm::vec3(node->aabb_.getMax().x, node->aabb_.getMax().y,
        //                                                                split_plane_list[k + 1].offset_));
        //                                 break;
        //                         }
        ...
        //                         if ((*primitives_)[node->getActualPrimitiveId(i)]->clipAgainstAABB(
        //                                 clipping_aabb, dummy, new_aabb, centroid)) {
        ...
        //                         } else {
        //                             split_plane_list[i].left_aabb_ =
        //                                 split_plane_list[i - 1].left_aabb_ + bin_list[i - 1].aabb_;
        //                             split_plane_list[i].left_area_ = split_plane_list[i].left_aabb_.getArea();
        //                             split_plane_list[i].num_left_prim_refs_ =
        //                                 split_plane_list[i - 1].num_left_prim_refs_ +
        //                                 bin_list[i - 1].in_primitive_refs_.size();
        //                         }
        //                     } else {
        ...
        //                         } else {
        //                             split_plane_list[i].right_aabb_ =
        //                                 split_plane_list[i + 1].right_aabb_ + bin_list[i].aabb_;
        //                             split_plane_list[i].right_area_ = split_plane_list[i].right_aabb_.getArea();
        //                             split_plane_list[i].num_right_prim_refs_ =
        //                                 split_plane_list[i + 1].num_right_prim_refs_ +
        ...
        //                     this_cost =
        //                         SAH(split_plane_list[i].num_left_prim_refs_, split_plane_list[i].left_aabb_.getArea(),
        //                             split_plane_list[i].num_right_prim_refs_, split_plane_list[i].right_aabb_.getArea(),
        //                             node->aabb_.getArea());
        //                     // root_aabb_area);

        //                     split_plane_list[i].sah_cost_ = this_cost;

        //                     if (child_side == "root->L->R->R->L->R->L->L->L->L->R->R") float dummy = this_cost;
        ...
        //     // if (false) { // force for SPATIAL SPLIT
            ...
            for (unsigned long int i = 0; i < node->left_node_->primitive_ref_list_->size(); ++i) {
                if (!i)
            ...
            for (unsigned long int i = 0; i < node->right_node_->primitive_ref_list_->size(); ++i) {
                if (!i)
        ...
        //     // node->left_node_->primitive_ref_list_ = SBVHNode::PrimitiveRefListUniquePtr(new
        ...
        //             if ((*primitives_)[node->getActualPrimitiveId(ref_id)]->clipAgainstAABB(
        //                     node->left_node_->aabb_, dummy, new_aabb, centroid)) {
        ...
        //             if ((*primitives_)[node->getActualPrimitiveId(ref_id)]->clipAgainstAABB(
        //                     node->right_node_->aabb_, dummy, new_aabb, centroid)) {
        ...
        node->left_node_->aabb_.setMin(node->left_node_->aabb_.getMin() - 1e-3f);
        node->left_node_->aabb_.setMax(node->left_node_->aabb_.getMax() + 1e-3f);
...
bool SBVH::traverse(const std::unique_ptr<SBVHNode>& node, const Ray& ray, IntersectionRecord& intersection_record,
    ...
    if (node) {
        if ((!node->left_node_) && (!node->right_node_)) {  // is a leaf node
            ...
            if (node->getNumReferences() == 1) {  // node contains 1 triangle
                ...
                if (primitives_[(*node->primitive_ref_list_)[0].id_]->intersect(
                        ray, tmp_intersection_record)) {  // if intersects...
                    ++num_intersections_;
                ...
                if (node->aabb_.intersect(ray)) {
                    ...
                    for (std::size_t primitive_id = 0; primitive_id < node->getNumReferences(); primitive_id++) {
                        ++num_intersection_tests;
                        if (primitives_[(*node->primitive_ref_list_)[primitive_id].id_]->intersect(
                                ray, tmp_intersection_record)) {
                            ++num_intersections;
                            if ((tmp_intersection_record.t_ < intersection_record.t_) &&
                                (tmp_intersection_record.t_ > 0.0f)) {
                                intersection_record = tmp_intersection_record;
                                primitive_intersect = true;
                            }
                        }
...
void SBVH::collectTreeStatistics() {
    ...
    if (root_node_) {
        ...
        while (!queue.empty()) {
            ...
            if (node) {
                ...
                if (node->left_node_) queue.push(node->left_node_.get());
                if (node->right_node_) queue.push(node->right_node_.get());
            } else if (!queue.empty()) {
                queue.push(nullptr);
    ...
    for (unsigned int i = 0; i < primitives.size(); ++i)
        if (primitives[i] == 0) ++statistics_.num_unreferenced_primitives_;
    ...
    // std::clog << "  Number of object / spatial splits: " << statistics_.num_object_splits_ << " / " <<
    ...
    if (root_node_) {
        ...
        while (!queue.empty()) {
            ...
            if (node) {
                ...
                output_file << " " << node->aabb_.getMin().x << " " << node->aabb_.getMin().y << " "
                            << node->aabb_.getMin().z;
                output_file << " " << node->aabb_.getMax().x << " " << node->aabb_.getMax().y << " "
                            << node->aabb_.getMax().z;
                output_file << "\n";
