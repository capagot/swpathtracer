...
const float Triangle::kIntersectionTestEpsilon_ = 1e-6f;
...
AABB Triangle::getAABB() const {
    ...
    aabb.setMin(glm::min(glm::min(v_[0], v_[1]), v_[2]));
    aabb.setMax(glm::max(glm::max(v_[0], v_[1]), v_[2]));
...
bool Triangle::clipAgainstAABB(const AABB& aabb, std::vector<glm::vec3>& new_vertex_list, AABB& new_aabb,
    ...
    splitPolygon(v_, Plane(glm::vec3(-1.0f, 0.0f, 0.0f), aabb.getMin()), tmp_vertex_list);
    splitPolygon(tmp_vertex_list, Plane(glm::vec3(0.0f, -1.0f, 0.0f), aabb.getMin()), new_vertex_list);
    tmp_vertex_list.clear();
    splitPolygon(new_vertex_list, Plane(glm::vec3(0.0f, 0.0f, -1.0f), aabb.getMin()), tmp_vertex_list);
    new_vertex_list.clear();
    splitPolygon(tmp_vertex_list, Plane(glm::vec3(1.0f, 0.0f, 0.0f), aabb.getMax()), new_vertex_list);
    tmp_vertex_list.clear();
    splitPolygon(new_vertex_list, Plane(glm::vec3(0.0f, 1.0f, 0.0f), aabb.getMax()), tmp_vertex_list);
    new_vertex_list.clear();
    splitPolygon(tmp_vertex_list, Plane(glm::vec3(0.0f, 0.0f, 1.0f), aabb.getMax()), new_vertex_list);
    ...
    for (unsigned int i = 0; i < new_vertex_list.size(); ++i) {
        ...
        if (i == 0) {
            new_aabb.setMin(new_vertex_list[i]);
            new_aabb.setMax(new_vertex_list[i]);
        } else {
            new_aabb.setMin(glm::min(new_aabb.getMin(), new_vertex_list[i]));
            new_aabb.setMax(glm::max(new_aabb.getMax(), new_vertex_list[i]));
        }
    }
...
void Triangle::splitPolygon(const std::vector<glm::vec3>& vertex_list, const Plane& plane,
    ...
    /* Robust triangle splitting algorithm adapted from the algorithm
     * presented in the Section 8.3.4 ("Splitting Polygons Against a Plane") of the book
     *
    ...
    if (vertex_list.size() > 0) {
        ...
        for (const glm::vec3& point_b : vertex_list) {
            ...
            if (b_side == PointPosition::IN_FRONT) {
                ...
                new_vertex_list.push_back(point_b);
            else if (a_side == PointPosition::ON_PLANE)  // test added to the origial algorithm
                new_vertex_list.push_back(point_a);
